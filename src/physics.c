#include "physics.h"
#include "sprites.h"
void spriteVectorInit(tMover* pMover,const UBYTE ubSpriteIndex,const int iLx, const int iLy, fix16_t iSx, const int iSy,const unsigned int uiMass)
{
...
void moverBounce(tMover* pMover)
{
  ...
  if (g_ubVBounceEnabled && (y>s_pAceSprites[pMover->ubSpriteIndex].iBounceBottomLimit-30||y<10))
  {
...
UBYTE moverMove(tMover sMover)
{
  ...
  UWORD uwLocationXTrail = uwLocationX + s_pAceSprites[sMover.ubSpriteIndex].uwSpriteCenter;
  UWORD uwLocationYTrail = uwLocationY + s_pAceSprites[sMover.ubSpriteIndex].uwSpriteCenter;
