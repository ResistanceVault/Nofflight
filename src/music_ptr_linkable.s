...
_drawpixel:
	...
	;lea pixelcolors+2,a6
	;move.b (a6),(a2)
;rts;
	clr.w $100 ; debug , to reall this just type w 0 0100 2 in fsuae debugger
	MOVE.l  #300-1,D1 
dp_loopy:
...
_calcpixel:
	...
	; a0 will point to the result address
	lea balldistances,a0

	; a2 and d2-d3 will be used for comparing x positions
	lea ballpos,a2
	move.b (a2),d2

	; a4 and d4-d5 will be used for comparing y positions
	lea ballpos+1,a4
	move.b (a4),d4

	move.l 	#40-1,d0 ; x resolution
	

loopx:
	MOVE.l  #32-1,D1 ; y resolution 

; start looping each pixel
loopy:
	move.b d0,d3			; for now let's pretend we are processing pixel at x = 5
	
	move.b d1,d5			; compute pixel with y coord d1

	; start calculating X abs value
	sub.b d2,d3
	bpl.b noneg 			; this will branch if the N flag is NOT set (the result is negative)
	neg.b d3
noneg:

	; start calculating Y abs value
	sub.b d4,d5
	bpl.b noneg2 			; this will branch if the N flag is NOT set (the result is negative)
	neg.b d5
noneg2:

	; add the 2 values to get the distance
	add.b d3,d5		# Now we have the distance in d5

	; store the distance in memory
	move.b d5,(a0)+

	; start of selecting where the copperlist must be modified
	;move.l d0,d3 ; copy X coord in d3 
	;move.l d1,d6 ; copy Y coord in d7

	;muls.w #180,d6 ; y poiniting

	;muls.w #4,d3 ; x pointing
...
nomove:
	...
	movem.l (sp)+,d1-d6/a0-a6
	move.l #balldistances,d0
	rts

ballpos: dc.b $10,$14
balldistances: dcb.b 40*32
pixelcolors: dc.w $00F0,$00F1,$00F2,$00F3,$00F4,$00F5,$00F6,$00F7,$00F8,$00F9,$00FA
